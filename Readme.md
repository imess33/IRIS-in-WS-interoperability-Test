...
# 互联互通评测的简单介绍
互联互通评测是一个复杂的项目， 要求医院对整个信息系统进行广泛的改造。 除了对医院业务的信息化要求(或者说定性的部分)，在基础技术层面,也就是定量部分定义了互联互通的数据元，数据集，接口和文档，是前期实验室评测的关键。这里我只谈定量部分的内容。 

- 数据元和数据集

应用于所有互联互通的文档和服务，通常也只会在文档和服务中检验。也就是说，文档和服务的消息会被检查其中的内容使用了数据元和数据集，至于被检验的业务系统或者集成平台的内部数据结构，并不要求和一致或者对应。  
...
需要熟悉IRIS的互连接性，或者说， HealthConnect的production，接口的创建，业务规则，BPL， 数据转换工具等等,这些技术工具都是做这个项目的必须。尤其是下面的内容，非常关键，如果客户在其他IRIS项目中没有接触过，那么做这个项目必须学会。  

+ **XPATH**: 通常用于从XML取值，用于路由判断。在不容易获得schema的情况下， XPATH是需要的技术。

+ **XLST**: 对于xml到CDA还是HL7 V3消息的转换， XSLT是最好的选择。使用它效率高， 不需要创建object。XSLT文件的产生可以使用任何的3方工具。

+ **虚拟文档（Virtural document）**: 对于HL7 V3, ISC的虚拟文档是最方便的工具， 简单易用且高效。

另外， 还有些技术，工具不是必须的， 但可能对大多数客户有帮助，或者对某一部分客户有需要，示例包中也包含了少量的相关内容。如下：  
 ...
 *注：示例中不包括hl7 v3的校验。 一般情况下在集成平台中不需要也不适合做HL7V3的校验*
